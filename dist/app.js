// 在head 中 加载 必要静态
// Dplayer
document.write('<script src="' + window.cdnurl + '/npm/hls.js@1.0.7/dist/hls.min.js"></script>');
document.write('<script src="' + window.cdnurl + '/npm/dashjs@4.0.1/dist/dash.all.debug.min.js"></script>');
document.write('<script src="' + window.cdnurl + '/npm/flv.js@1.6.1/dist/flv.min.js"></script>');
document.write('<script src="' + window.cdnurl + '/npm/cdnbye@latest"></script>');
document.write('<script src="' + window.cdnurl + '/npm/dplayer@1.26.0/dist/DPlayer.min.js"></script>');
// plyr
document.write('<script src="' + window.cdnurl + '/gh/video-dev/hls.js@18bb552/dist/hls.min.js"></script>');
document.write('<script src="//cdn.dashjs.org/latest/dash.all.min.js"></script>');
document.write('<script src="' + window.cdnurl + '/npm/plyr@3.6.8/dist/plyr.min.js"></script>');
// markdown支持
...
if (ThemeConfig.menu_show) {
    for (let i = 0; i < ThemeConfig.menus.length; i++) {
        var iconOfMenu = ThemeConfig.menus[i].icon != undefined ? ThemeConfig.menus[i].icon : 'attachment';
        html += `<a href="${ThemeConfig.menus[i].url}" target="_blank"><i class="mdui-icon material-icons sidenavIcon">${iconOfMenu}</i>${ThemeConfig.menus[i].name}</a>`;
    }
}
html += `
  </div>
...
function append_files_to_list(path, files) {
    ...
    for (i in files) {
        ...
        if (item['mimeType'] == 'application/vnd.google-apps.folder') {
            html += `<li class="mdui-list-item mdui-ripple"><a href="${p}" class="folder">
              ...
              <i class="mdui-icon material-icons">folder</i>
                ...
                <div class="file-info">${item['modifiedTime']}</div>
              </div>
		    ...
		    $("#readme_md").wrap("<div class='cannem-item'></div>");
                });
            }
            if (item.name == "HEAD.md") {
                get_file(p, item, function(data) {
                    markdown("#head_md", data);
		    $("#head_md").wrap("<div class='cannem-item'></div>");
                });
            }
		...
		c += " view";
	        ...
	        p = `https://docs.google.com/document/d/${item.id}`
	    }
            html += `<li class="mdui-list-item file mdui-ripple" target="_blank"><a gd-type="${item.mimeType}" href="${p}" class="${c}">
            <div class="mdui-col-xs-12 mdui-col-sm-7 mdui-text-truncate" title="${item.name}">
	    <i class="mdui-icon material-icons">`
	    if ("|bmp|jpg|jpeg|png|gif|".indexOf(`|${ext}|`) >= 0) {       //添加不同的图标
		html += `photo`
	    }
	    else if ("|md|".indexOf(`|${ext}|`) >= 0) {
	    	html += `description`
	    }
            else if ("|mp4|webm|avi|mpg|mpeg|mkv|rm|rmvb|mov|wmv|asf|ts|flv|m3u8|".indexOf(`|${ext}|`) >= 0) {
		html += `movie_creation`
	    }
	    else if ("|mp3|flac|wav|ogg|m4a|".indexOf(`|${ext}|`) >= 0) {
		html += `music_note`
	    }
	    else {
		...
		html += `<div class="thumIcon-father"><img class="thumbIcon" onload="this.parentNode.style.opacity=1" src="${item['thumbnailLink']}"/></div>`;                   //添加预览图
	    }
...
function onSearchResultItemClick(a_ele) {
    ...
    $.post(`/${cur}:id2path`, { id: a_ele.id }, function(data) {
        ...
        dialog = mdui.dialog({
            title: '获取目标路径失败',
            content: ' 可能是因为网盘中已经删除此项，或是目标是未被拷贝的“与我共享”内容',
            history: false,
            modal: true,
            closeOnEsc: true,
            buttons: [
...
function file(path) {
    ...
    if ("|md|".indexOf(`|${ext}|`) >= 0) {
    	return file_markdown(path);
    }
    if ("|php|css|go|java|js|json|txt|sh|html|".indexOf(`|${ext}|`) >= 0) {
...
function file_code(path) {
    ...
    var name = path.split('/').pop();
    var ext = name.split('.').pop().toLowerCase();
    var href = decodeURI(window.location.origin + path);
    var content = `
<div class="mdui-container">
<pre id="editor" ></pre>
</div>
...
function file_video(path) {
    const url = decodeURI(window.location.origin + path);
        ...
        if (target_children.length > 0 && target_children.includes(path)) {
            ...
            <div class="mdui-container">
                <div class="mdui-row-xs-2 mdui-m-b-1">
                    <div class="mdui-col">
                        ${prev_child ? `<button id="leftBtn" onclick="location='${prev_child}?a=view'" class="mdui-btn mdui-btn-block mdui-color-theme-accent mdui-ripple">上一张</button>` : `<button class="mdui-btn mdui-btn-block mdui-color-theme-accent mdui-ripple" disabled>上一张</button>`}
                    </div>
                    <div class="mdui-col">
                        ${next_child ? `<button id="rightBtn" onclick="location='${next_child}?a=view'" class="mdui-btn mdui-btn-block mdui-color-theme-accent mdui-ripple">下一张</button>` : `<button class="mdui-btn mdui-btn-block mdui-color-theme-accent mdui-ripple" disabled>下一张</button>`}
                    </div> 
                </div>
            </div>
            `;
